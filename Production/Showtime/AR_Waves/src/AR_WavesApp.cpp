...
void ARWaves::setup()
{
	...
	setFrameRate(60.0f);
	//setup shader
	mShader = gl::GlslProg::create(loadAsset("shaders/waves_render_vert.glsl"),
		loadAsset("shaders/waves_render_frag.glsl"));
...
void ARWaves::updateWaves(float frames)
{
	int id = 0;
...
void ARWaves::updateOrb(float frame)
{
	int id = 0;
...
CINDER_APP(ARWaves, RendererGl(RendererGl::Options().msaa(16)))
